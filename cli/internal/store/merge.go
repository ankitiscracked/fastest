...
func (s *Store) PlanMerge(currentSnapshotID, sourceSnapshotID string, force bool) (*MergePlan, error) {
	...
	toApply, conflicts, inSyncCount := computeMergeActions(baseManifest, currentManifest, sourceManifest)
...
// For each file path, it determines whether to apply from source, flag as conflict,
// or skip (already in sync).
func computeMergeActions(base, current, source *manifest.Manifest) (toApply, conflicts []MergeAction, inSync int) {
	// Build lookup maps
	...
	return toApply, conflicts, inSync
}
